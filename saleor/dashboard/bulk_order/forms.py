...
from ...core.models import BulkOrderColor, BulkOrderType
...
class OrderTypeChoiceValueForm(forms.ModelForm):
    class Meta:
        model = BulkOrderType
        fields = ['name']
        labels = {
...
class OrderColorChoiceValueForm(forms.ModelForm):
    class Meta:
        model = BulkOrderColor
        fields = ['name']
        labels = {
            'name': pgettext_lazy(
                'Item name', 'Name')}
